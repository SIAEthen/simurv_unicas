...
t_f = 10;       % final simulation time [s]
...
for i=1:npti
    ...
    if (KinOnly==1)
        ...
        %use a observer 
        if(i>2)
            [tau_e(:,i),sigma_observer(:,i+1)] = observer_f_t_xinhui(eta(:,i),gamma_v(:,i),tau_e(:,i-1),dzita(1:3,i),zita(4:6,i),tau(:,i-1),sigma_observer(:,i),Ts);
        end
