...
for i=1:npti
    ...
    if (KinOnly==1)
        ...
        target(:,i) = obj_pos + 2*[sin(.4*t(i)); cos(.4*t(i)); 0];
        %sigma_tilde_a(:,i) = sigma_tilde07(target(:,i),eta(:,i),[DH(:,1:3) q(:,i)],PARAM.T_0_B);
        %J_a = J07(target(:,i),eta(:,i),[DH(:,1:3) q(:,i)],PARAM.T_0_B);
        %k_a = 5;
        ...
        sigma_tilde_c(:,i) = sigma_tilde16(PARAM.q_min,PARAM.q_max,q_offset,q_i,q(:,i));
        J_c = J16(PARAM.q_min,PARAM.q_max,q_offset,q_i,q(:,i));
