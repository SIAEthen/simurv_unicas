...
W = diag([100 100 100 50 50 50 5*ones(1,4) 1*ones(1,3)]);
invW=inv(W);
...
for i=1:npti
    ...
    eta_ee1_d(:,i) = trapezoidal2(eta_ee1(:,1),obj_pos-[0 0 .5*obj_size]',[.5 .5 .5]',10,t(i));
    eta_ee_quat_d(:,i) = GenerateDesQuat(eta_ee_quat(:,1),R_d,.7,12,t(i));
    target(:,i) = obj_pos + 1*[sin(.4*t(i)); cos(.4*t(i)); 0];
    ...
    if (KinOnly==1)
        ...
        %k_a = 20;
        % first priority task - field of view
        sigma_tilde_a(:,i) = sigma_tilde17(target(:,i),eta(:,i),[DH(:,1:3) q(:,i)],PARAM.T_0_B);
        J_a = J17(target(:,i),eta(:,i),[DH(:,1:3) q(:,i)],PARAM.T_0_B);
        k_a = 5;
        ...
        k_b = 10;
        % third-priority task - robot nominal configuration (only second joint)
        J_ab = [J_a; J_b];
        pinvJ_ab = mypinv(J_ab,invW);
        N_ab = eye(6+n,6+n)-pinvJ_ab*J_ab;
        sigma_tilde_c(:,i) = sigma_tilde08(q(:,i),PARAM.q_nominal(2),2);
        J_c = J08(q(:,i),2);
        k_c = 10;
        pinvJ_c = mypinv(J_c,invW,'c');
        % fourth-priority task - far from mechnical joint limits
        J_abc = [J_a; J_b; J_c];
        pinvJ_abc = mypinv(J_abc,invW);
        N_abc = eye(6+n,6+n)-pinvJ_abc*J_abc;
        sigma_tilde_d(:,i) = sigma_tilde16(q_min,q_max,.2*ones(n,1),.2*ones(n,1),q(:,i));
        J_d = J16(q_min,q_max,.2*ones(n,1),.2*ones(n,1),q(:,i));
        k_d = 2;
        pinvJ_d = mypinv(J_d,invW,'d');
        ...
        zita_d(:,i) =      pinvJ_a*k_a*sigma_tilde_a(:,i) + ...
                        N_a*pinvJ_b*k_b*sigma_tilde_b(:,i) +...
                       N_ab*pinvJ_c*k_c*sigma_tilde_c(:,i) + ...
                       N_abc*pinvJ_d*k_d*sigma_tilde_d(:,i);
