...
r_B0_B = [0 0 1.5]'; % vector position from origin of frame B to the
...
% D-H table
DH_a     = [.15   .61  .11    0     0    0]';   % [m]
DH_alpha = [-pi/2  0  -pi/2 pi/2  -pi/2  0]';   % [rad]
DH_d     = [  0    0    0    .61  -.113 .103]'; % [m]
DH_theta = [0 0 0 0 0 0]'/180*pi; % [rad]
...
r_c  = [0      0.11   0.055  0     0      0
        0.425  0      0      0.21  0.055  0
        0      0      0      0     0     -0.05];      % m
...
r_l = [.2 .1 .15 .1 .15 .1];  % link radius [m]
L_l = [.85 1 .2  .8 .2  .3];  % link length [m]
delta = pi*(r_l.^2).*(L_l);   % link volumes [m^3]
m = [80 80 30 50 20 25];      % link masses [kg]
...
Ma(:,:,4) = diag([ rho*delta(4), 0.1*m(4), rho*delta(4)]);
...
I(:,:,2) = 20*diag([1 4 4]);
I(:,:,3) = 20*diag([.1 .025 .1]);
I(:,:,4) = 20*diag([2.5 .45 2.5]);
I(:,:,5) = 20*diag([.25 .2 .25]);
I(:,:,6) = 20*diag([.25 .25 .17]);
...
Ia(:,:,4) = diag([(pi*rho*r_l(4)^2*L_l(4)^2)/12, 0, (pi*rho*r_l(4)^2*L_l(4)^2)/12]);
...
fric_dry = 0*[20 10 5  10 5 6];
% viscous friction of the links
fric_vis = [30 20 5 10 5 6];

Ds = 0.4;   % linear skin coefficient
CD = 0.6;   % quadratic drag coefficient
CL = 0;		% lift coefficient

zita_limit = [.2 .2 .2 .2 .2 .2 [20 20 20 20 20 20]/180*pi];
q_nominal = [0 -45 0 0 0 0]/180*pi;
