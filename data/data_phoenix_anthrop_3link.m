%
% Phoenix + 3 link anthropomorphous
%
...
Ma = zeros(3,3,3);
Ma(:,:,1) = diag([ rho*delta(1), rho*delta(1), 0.1*m(1)]);
...
Ia = zeros(3,3,3);
Ia(:,:,1) = diag([(pi*rho*r_l(1)^2*L_l(1)^2)/12, (pi*rho*r_l(1)^2*L_l(1)^2)/12, 0]);
